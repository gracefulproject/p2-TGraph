...
class sdpComm(QtCore.QObject):
    ...
    def __init__(self, chipList, machineIP, parent=None):
    ...
    def sendSDP(self, flags, tag, dp, dc, dax, day, cmd, seq, arg1, arg2, arg3, bArray):
        ...
        CmdSock.writeDatagram(sdp, QtNetwork.QHostAddress(self.machineIP), DEF_SEND_PORT)
    ...
    def sendPing(self, map):
        for node in map:
            if node != -1:
                x,y = getChipXYfromID(map, node, self.chipList)
    ...
    def sendSourceTarget(self, map, srcTarget):
        ...
        xSrc, ySrc = getChipXYfromID(map, DEF_SOURCE_ID, self.chipList)    # get the x and y of SOURCE node
        bArray = list()
        for node in srcTarget:  # then node is the "key" in the dictionary
            x,y = getChipXYfromID(map, node, self.chipList)                # get the x and y of the target node of the SOURCE
            bArray.append(node)
            bArray.append(x)
            bArray.append(y)
    ...
    def sendConfig(self, map, cfg):
        ...
        for target in cfg:  # each "target" is a dict
            ...
            dax,day = getChipXYfromID(map, nodeID, self.chipList)
            if dax==-1 or day==-1:
                print "Invalid chip coordinate for nodeID-",nodeID, "in map",map
            nPkt = target['nPkt']
    ...
    def sendChipMap(self, map):
        ...
        xSrc, ySrc = getChipXYfromID(map, DEF_SOURCE_ID, self.chipList)
        ...
        for item in map:
            if item==DEF_SOURCE_ID or item==-1: # we skip SOURCE because it is already in arg
                ...
                nNode += 1
                x,y = getChipXYfromID(map, item, self.chipList)
