...
/* hSDP_monitor()
 * For root monitor, it has the following additional tasks:
 * - receives network structure from host-PC and do initial mapping
 * - invokes optimizer to do dynamic mapping later on
 * - do process migration
 *
 * When requesting allocation, host-PC send the following information:
 * - cmd_rc
 * */
...
void c_main()
{
	...
	// for all monitor core, it has the basic functionality: monitoring traffic:
	printP2Ptable();
	// we need to modify the diagnostic counter:
	...
	spin1_callback_on(TIMER_TICK, hTimer_monitor, PRIORITY_TIMER);

	// initialization
	numRunningTG = 0;	// no TG in the beginning

	/* for root-monitor, it has bigger roles:
	   - receives network structure from host-PC and do initial mapping
