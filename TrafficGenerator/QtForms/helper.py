...
def getChipXYfromID(TGmap, TGNodeId, table):
...
def getChipIDfromXY(line):
    """
    This is used primarily for reading TGmap configuration file
    :param line: (x,y)=id
    :return: 
    """
    ...
    line = line.strip()
    if line[0]=='#':    # 'sharp' sign indicate a comment line
        return None
    ...
    try:
        ...
        xy = term[0].split(',')
        if len(xy) != 2:
            print "Found invalie line:", line
            return None
        # Remove non numeric characters:
        x = int(''.join(c for c in xy[0] if c.isdigit()))
        y = int(''.join(c for c in xy[1] if c.isdigit()))
        id = int(''.join(c for c in term[1] if c.isdigit()))
        dct = dict()
        dct[(x,y)]=id
        return dct
    except ValueError:
...
def getDeadChipList(lst):
    """
    produce a list of tuples of XY coordinates of dead chips
    :param line: lst is a string containing list of dead chips
                 example: (0,2) (0,4)
    :return: 
    """
    # TODO: lanjutkan ini..., sementara ini di bypas dulu
    result = [(0,2)]
    return result
