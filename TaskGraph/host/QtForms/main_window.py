...
class MainWindow(QtGui.QMainWindow, QtMainWindow.Ui_qtMainWindow):
    """
    We can feed this class with the argv from sys (called from the main tgsdp.py)
    """
    def __init__(self, argList, parent=None):
        ...
        # if the ip address is provided as a command line argument, use it. Otherwise, ask it here
        self.spinIP = None
        if len(argList)==1:
            self.spinIP, ok = QtGui.QInputDialog.getText(None, "SpiNNaker Machine", "Specify Machine IP address",
                                                    QtGui.QLineEdit.Normal, DEF_HOST)
            if ok is True:
                print "Will connect to", self.spinIP
                self.statusTxt.setText("Using machine at {}".self.spinIP)
            else:
                print "Will connect to a machine later"
                self.spinIP = None
        else:
            self.spinIP = argList[1]

        if self.spinIP is not None:
            self.connect2Machine()
    ...
    def sendAndInit(self):
        ...
        print "Send iptag...",
        self.mc.iptag_set(DEF_TUBO_IPTAG, MY_PC_ADDR, DEF_TUBO_PORT,0,0) # prepare iptag for myTub, because boot in rig doesn't provide this
        self.mc.iptag_set(DEF_REPORT_IPTAG, MY_PC_ADDR, DEF_REPORT_PORT,0,0)
        print "done!"
        ...
        """
        srcsinkaplx = DEF_SRCSINK_APLX
        tgsdpaplx = DEF_TGSDP_APLX
        monaplx = DEF_MONITOR_APLX
